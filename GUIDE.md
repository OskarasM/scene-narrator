...
- Live demo: <https://scene-narrator-inky.vercel.app/>. Three scenes, live controls, and a running transcript of every sentence the library writes. Built from `demo/` on every push to main, so it is always the commit above.
